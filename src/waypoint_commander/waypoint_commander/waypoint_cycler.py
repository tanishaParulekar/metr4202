...
class WaypointCycler(Node):
    ...
    def __init__(self):
        ...
        # Create a publisher
        # The constructor inputs are (message type, topic name, 
        ...
        # Best practice would be to put these in a config file that's 
        # read in as params
        p0 = PoseStamped()
    ...
    def behavior_tree_callback(self, msg:BehaviorTreeLog):
        # Check the current state of the behavior tree
        # When the robot has reached its waypoint, the top node state is
        # "navigateRecovery" and the event status is 'IDLE'
        # If the behavior tree is in this state, send the next waypoint
